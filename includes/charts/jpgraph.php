...
use charts\jpgraph\PieGraph;
use charts\jpgraph\text;
use charts\jpgraph\RotImage;
use charts\jpgraph\ImgStreamCache;
use charts\jpgraph\Legend;
use charts\jpgraph\vividTheme;
use charts\jpgraph\universalTheme;
use charts\jpgraph\LinePlot;
use charts\jpgraph\BarPlot;
use charts\jpgraph\AccBarPlot;
use charts\jpgraph\GroupBarPlot;
use charts\jpgraph\Gradient;
use charts\jpgraph\PiePlot3D;
use charts\model;
...
require_once('jpgraph/jpgraph_theme.inc.php');
...
class jpgraph extends model
{
    ...
    function __construct($data, $imageDir)
    {
        $this->data = $data;
    ...
    static function setData($charts, $data)
    {
        $imageDir = $charts->getImageDir();
        return new static($data, $imageDir);
    }
    ...
    {
        $xTitle = getDefault($params['xTitle']);
        $yTitle = getDefault($params['yTitle']);
        $filename = getDefault($params['filename']);
        $autoScale = getDefault($params['auto']);
        $offSetHieght = getDefault($params['offSetHieght']);
        
        $colors = [
            'Reserved' => '#FFA500',
            'Order Processing' => '#0000FF',
            'Discrepancy' => '#00FF00',
            'Shipping' => '#FFFF00',
            'Received' => '#FF69B4',
            'Bill Of Ladings' => '#F4A460',
            'Shipping Check-In' => '#AFEEEE',
            'Processing Check-In' => '#00FFFF',
            'Processing Check-Out' => '#C71585',
            'Entry Checked-In' => '#9ACD32',
            'Entry Check-Out' => '#FFFF00',
            'Picking Check-In' => '#0000CD',
            'Picking Check-Out' => '#FFC0CB'
        ];
        ...
        switch ($params['type']) {
            case 'pie':
                $graph = new PieGraph($params['width'], $params['height']);
                break;
            default:
                $graph = $autoScale ? 
                    new Graph($params['width'], $params['height'], 'auto') : 
                    new Graph($params['width'], $params['height']);
        }
        ...
        //graph
        $graph->SetMargin(90, 80+$offSetHieght, 70, 130);
        $graph->SetBackgroundGradient('#D4E6F1', '#EAF2F8');
        ...
        $yTitle ? $graph->yaxis->title->Set($yTitle) : NULL;
        ...
        $xTitle ? $graph->xaxis->SetTitle($xTitle,'center') : NULL;
        ...
        switch ($params['type']) {
            case 'pie':
                $groupCounts = 
                    array_column($this->data, $params['groupField']);
                $pie = new PiePlot3D($groupCounts);
                $legends = array_keys($this->data);
                ...
                break;
            case 'line':
                $plotResults = $this->calcPlots($params);
                $graph->xaxis->SetTickLabels($plotResults['xData']);
                foreach ($plotResults['plots'] as $wh => $plots) {
                    $point = new LinePlot($plots);
                    $graph->Add($point);
                    $point->SetLegend($wh);
                }
            ...
            case 'bar':
                ...
                $plotResults = $this->calcPlots($params);
                $graph->xaxis->SetTickLabels($plotResults['xData']);
                foreach ($plotResults['plots'] as $plot) {
                    $groupBarPlots[] = new BarPlot($plot);
                }
 
                $gbplot = new GroupBarPlot($groupBarPlots);
                $graph->Add($gbplot);

                foreach ($groupBarPlots as $index => $plot) {
                    $plot->SetLegend($plotResults['groups'][$index]);
                }
                break;
            case 'verticalbar':
                $accBarPlots = [];
                ...
                foreach ($plotResults['plots'] as $status => $plot) {
                    $barPlot = new BarPlot($plot);
                    $barPlot->SetFillColor($colors[$status]);
                    $accBarPlots[] = $barPlot;
                }

                // Create the accumulated bar plots 
                $accBarPlot = new AccBarPlot($accBarPlots);

                //bar width
                if (count($plotResults['xData']) < 5) {
                    $accBarPlot->SetWidth(0.07);
                } else if (count($plotResults['xData']) >= 5 
                        && count($plotResults['xData']) < 10) {
                    $accBarPlot->SetWidth(0.4);
                } else if (count($plotResults['xData']) >= 10 
                        && count($plotResults['xData']) < 17) {
                   $accBarPlot->SetWidth(0.6);
                } else {
                    $accBarPlot->SetWidth(0.9);
                }

                //add accumulated bar plot to graph
                $graph->Add($accBarPlot);
               
                //add report date to graph
                $txt = new Text('REPORT DATE : ' . date('Y-m-d'));
                $txt->SetPos(0, 5);
                $txt->SetColor('blue');
                $txt->SetFont(FF_FONT1, FS_BOLD, 1);
                $graph->AddText($txt); 
               
                //add legends
                foreach ($accBarPlots as $index => $plot) {
                    $plot->SetLegend($plotResults['groups'][$index]);
                }
        ...
        // Output line
        $imageFile = $this->imageDir.'/'.$params['filename'].'.png';

        $filename ? $graph->Stroke(_IMG_HANDLER) : $graph->Stroke();
        $filename ? $graph->img->Stream($imageFile) : NULL;
    }
}
            ...
            if( substr($this->axtype,0,4) == "text" ) {
                ...
                for($i=0; $i < $n; ++$i ) {
                    ...
                    if( class_exists('BarPlot',false) ) {
                        $cl = strtolower(basename(get_class($p)));
