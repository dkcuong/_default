...
abstract class Theme 
{
    ...
    */
    function ApplyGraph($graph) {
        ...
        $className = get_class($graph);
        $basename = basename($className);
        if ($basename == 'Graph') {
            method_exists($this, 'SetupGraph') ? 
                $this->SetupGraph($graph) : 
                // Theme::%s() is not defined. \nPlease make %s(\$graph) 
                // function in your theme classs.
                JpGraphError::RaiseL(30001, 'SetupGraph', 'SetupGraph'); 
        } else {
            // Terrible...
            $method_name = 'Setup' . $basename;
