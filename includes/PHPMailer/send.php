...
class send
{
    ...
    public function __construct($exceptions = false)
    {
        ...
        self::$noReply = \appConfig::getMailConfig();
    }
     ...
     */
    static function jsonAPI($app)
    {
        unset($app->post['username'], $app->post['password']);
        
        return self::mail($app->post);
    }
    ...
    {      
        ...
        $recipient = isset($params['recipient']) ? $params['recipient'] : 
            $params['recipients'];
        $username = getDefault($params['username'], self::$noReply['username']);
        $password = getDefault($params['password'], self::$noReply['password']);
        $displayFrom = getDefault($params['displayFrom'], self::$noReply['display']);
        $addReplyTo = getDefault($params['addReplyTo']);

        $send->isSMTP();
        $send->Host = self::$noReply['host'];
        $send->SMTPAuth = TRUE;
        $send->Username = $username;
        $send->Password = $password;
        $send->SMTPSecure =
            getDefault($params['protocol'], self::$noReply['encryption']);
        $send->Port = getDefault($params['port'], self::$noReply['port']);
        $send->WordWrap = 50;
        $send->From = $username;
        $send->FromName = $displayFrom;
        ...
        // Adding address reply to
        if ($addReplyTo) {
            $send->addReplyTo($addReplyTo['email'], $addReplyTo['fullName']);
        }

        $file = getDefault($params['files']);
